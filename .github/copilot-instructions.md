...
```
AIPlantHealthCheck.sln
├── src/
│   ├── AIPlantHealthCheck.Domain/         # 领域层 - 聚合根、实体、领域事件
│   ├── AIPlantHealthCheck.Infrastructure/ # 基础设施层 - EF配置、仓储接口、仓储实现
│   └── AIPlantHealthCheck.Web/           # 表现层 - API、应用服务
└── test/                            # 测试项目
    ├── AIPlantHealthCheck.Domain.UnitTests/         # 领域层测试项目
    ├── AIPlantHealthCheck.Infrastructure.UnitTests/ # 基础设施层测试项目
    └── AIPlantHealthCheck.Web.UnitTests/           # 表现层测试项目
```
...
### 文件组织
- **聚合根** → `src/AIPlantHealthCheck.Domain/AggregatesModel/{AggregateFolder}/`
- **领域事件** → `src/AIPlantHealthCheck.Domain/DomainEvents/`
- **仓储** → `src/AIPlantHealthCheck.Infrastructure/Repositories/`
- **实体配置** → `src/AIPlantHealthCheck.Infrastructure/EntityConfigurations/`
- **命令与命令处理器** → `src/AIPlantHealthCheck.Web/Application/Commands/`
- **查询与查询处理器** → `src/AIPlantHealthCheck.Web/Application/Queries/`
- **API端点** → `src/AIPlantHealthCheck.Web/Endpoints/`
- **领域事件处理器** → `src/AIPlantHealthCheck.Web/Application/DomainEventHandlers/`
- **集成事件** → `src/AIPlantHealthCheck.Web/Application/IntegrationEvents/`
- **集成事件转换器** → `src/AIPlantHealthCheck.Web/Application/IntegrationEventConverters/`
- **集成事件处理器** → `src/AIPlantHealthCheck.Web/Application/IntegrationEventHandlers/`
...
**Web层** (`src/AIPlantHealthCheck.Web/GlobalUsings.cs`):
...
**Infrastructure层** (`src/AIPlantHealthCheck.Infrastructure/GlobalUsings.cs`):
...
**Domain层** (`src/AIPlantHealthCheck.Domain/GlobalUsings.cs`):
...
**查询处理器**:
```csharp
using AIPlantHealthCheck.Domain.AggregatesModel.{AggregateFolder};
using AIPlantHealthCheck.Infrastructure;
```
...
```csharp
using AIPlantHealthCheck.Domain.AggregatesModel.{AggregateFolder};
using AIPlantHealthCheck.Web.Application.Commands.{FeatureFolder};
using AIPlantHealthCheck.Web.Application.Queries.{FeatureFolder};
```
