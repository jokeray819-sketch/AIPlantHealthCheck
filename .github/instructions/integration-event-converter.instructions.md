---
applyTo: "src/AIPlantHealthCheck.Web/Application/IntegrationEventConverters/*.cs"
---
...
- 转换器应放置在 `src/AIPlantHealthCheck.Web/Application/IntegrationEventConverters/` 目录下。
- 转换器文件名格式为 `{Entity}{Action}IntegrationEventConverter.cs`。

## 代码示例

**文件**: `src/AIPlantHealthCheck.Web/Application/IntegrationEventConverters/UserCreatedIntegrationEventConverter.cs`

```csharp
using AIPlantHealthCheck.Domain.DomainEvents;
using AIPlantHealthCheck.Web.Application.IntegrationEvents;

namespace AIPlantHealthCheck.Web.Application.IntegrationEventConverters;
