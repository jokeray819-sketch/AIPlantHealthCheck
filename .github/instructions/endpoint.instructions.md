---
applyTo: "src/AIPlantHealthCheck.Web/Endpoints/**/*.cs"
---
...
- 类文件应放置在 `src/AIPlantHealthCheck.Web/Endpoints/{Module}/` 目录下。
- 端点文件名格式为 `{Action}{Entity}Endpoint.cs`。
- 请求 DTO、响应 DTO 与端点定义在同一文件中。

## 代码示例

**文件**: `src/AIPlantHealthCheck.Web/Endpoints/User/CreateUserEndpoint.cs`

```csharp
using FastEndpoints;
using AIPlantHealthCheck.Web.Application.Commands;
using AIPlantHealthCheck.Domain.AggregatesModel.UserAggregate;
using Microsoft.AspNetCore.Authorization;

namespace AIPlantHealthCheck.Web.Endpoints.User;
