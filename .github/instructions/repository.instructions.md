---
applyTo: "src/AIPlantHealthCheck.Infrastructure/Repositories/*.cs"
---
...
- 仓储接口和实现应放置在 `src/AIPlantHealthCheck.Infrastructure/Repositories/` 目录下。
- 文件名格式为 `{AggregateName}Repository.cs`。

## 代码示例

**文件**: `src/AIPlantHealthCheck.Infrastructure/Repositories/AdminUserRepository.cs`

```csharp
using AIPlantHealthCheck.Domain.AggregatesModel.AdminUserAggregate;
